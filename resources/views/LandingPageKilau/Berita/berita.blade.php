...
@section('style')
    <style>
        ...
        .tags-container{
            display:flex;
            flex-wrap:wrap;          /* biar otomatis turun baris */
            gap:8px 12px;            /* 8 px antar‑baris, 12 px antar‑kolom */
            margin-top:8px;          /* jarak ke atas (konten berita) */
        }
        
        .tag-link{
            display:inline-block;
            padding:6px 14px;        /* klik‑area lebih nyaman */
            font-size:.875rem;       /* 0.875 rem ≈ 14 px */
            line-height:1;           /* chip lebih kompak */
            color:#0d6efd;           /* Bootstrap primary */
            border:1px solid currentColor;
            border-radius:20px;
            text-decoration:none;
            transition:all .2s ease;
            background:#fff;         /* kontras di dark header */
        }
        ...
        @media (max-width:576px){      /* ekstra ruang di HP kecil */
            .tag-link{padding:6px 12px;font-size:.8rem;}
            .tags-container{gap:6px 8px;}
        }
            ...
            <div class="row g-5">
                    ...
                    <section id="blog-details" class="blog-details section">
                        <article class="article">
                           ...
                           {{--  @if (isset($berita['kategori']) && isset($berita['kategori']['name_kategori']))
                            ...
                            <!-- Container Tags di bawah konten berita -->
                            <div id="modal-tags" class="tags-container">
                                @if(isset($berita['tags']) && count($berita['tags']) > 0)
                                    @foreach($berita['tags'] as $tag)
                                        <a href="{{ $tag['link'] }}" target="_blank" class="tag-link">{{ $tag['nama'] }}</a>
                                    @endforeach
                                @else
                                    <!-- Jika tidak ada tags, Anda bisa menampilkan pesan atau biarkan kosong -->
                ...
                <div class="modal fade" id="donasiPrompt" tabindex="-1" aria-labelledby="donasiPromptLabel" aria-hidden="true">
                    ...
                    <div class="modal-content border-0 shadow">
                        ...
                        <p class="mb-3">
                            Setelah membaca berita ini, apakah Anda ingin berdonasi
                            untuk mendukung program program yang ada di <b>Kilau Indonesia</b>?
                        </p>
                ...
                @include('LandingPageKilau.Berita.partials-donasi', ['programs' => $programs])
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    <script>
         $(document).ready(function () {
            ...
            const promptM = bootstrap.Modal.getOrCreateInstance('#donasiPrompt', {
                backdrop: 'static',
                keyboard: false
            });
            const donasiM = bootstrap.Modal.getOrCreateInstance('#donasiModal');

            // Munculkan modal prompt setelah 30 detik
            setTimeout(() => {
                promptM.show();
            }, 30000); // 30.000 milidetik = 30 detik

            $('#promptDonasiYa').on('click', function () {
                promptM.hide();
                donasiM.show();
            });
                        ...
                        if (response.success && response.data) {
                            ...
                            let articleHtml = `<article class="article">
                                ...
                                <div class="content py-3" id="konten-detail">
                                   ...
                                   <div id="modal-tags" class="tags-container" style="display: flex; flex-wrap: wrap; gap: 5px;">
                                    ${tagsHtml}
                                  </div>
                            </article>`;
